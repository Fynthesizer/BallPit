...
public class controller : MonoBehaviour
{
    ...
    public GameObject currentTimbreToggle;
...
#if UNITY_ANDROID
        info.transform.GetChild(1).GetComponent<Text>().text = "Tap to throw a ball" + "\n" + "\n" + "- Use headphones -";
    ...
    void Update()
    {
        ...
        */
        //debugUI.text = "" + touchLength;

        foreach (Touch touch in Input.touches)
        {
            int id = touch.fingerId;
            if (id == 0 && !EventSystem.current.IsPointerOverGameObject(touch.fingerId))
            {
                ...
                {
                    if (touchLength < 0.5f && touchLength > 0.05f)
                    {
                        SpawnBall();
                    }
                    touchLength = 0f;
                }
                this.transform.Rotate(0, -touch.deltaPosition.x * cameraSpeed, 0);
                this.transform.parent.Rotate(touch.deltaPosition.y * cameraSpeed, 0, 0);
                xRotation += -touch.deltaPosition.x * cameraSpeed;
                yRotation += touch.deltaPosition.y * cameraSpeed;
                Mathf.Clamp(yRotation, -90f, 90f);
                this.transform.eulerAngles = new Vector3(0, xRotation, 0);
                this.transform.parent.eulerAngles = new Vector3(yRotation, 0, 0);
                */
            }
            else return;
        }
    ...
    */
    ...
    public void SpawnBall()
    {
        ...
        newBall.GetComponent<ball>().Setup(currentPitch, currentTimbre, ballCount);
        //newBall.GetComponent<ball>().pitch = currentPitch;
        //newBall.GetComponent<ball>().timbre = currentTimbre;

        newBall.GetComponent<Rigidbody>().AddForce(transform.forward * ballForce);
        ballCount++;
    ...
    public void ToggleReverb (bool state)
    {
        if(state) FMODUnity.RuntimeManager.StudioSystem.setParameterByName("ReverbAmount", 1);
        else FMODUnity.RuntimeManager.StudioSystem.setParameterByName("ReverbAmount", 0);
    }
